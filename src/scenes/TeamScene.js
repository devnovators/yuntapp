...
export default class TeamScene extends Component {
  ...
  componentDidMount() {
    ...
    firebase.database().ref('prueba/pesanchez_bbva_com').once('value').then((snapshot)=>{
      this.setState({
        name: snapshot.val().name,
        role: snapshot.val().role
      });
    });
    firebase.database().ref('prueba/pesanchez_bbva_com/peers').once('value').then((snapshot)=>{
      var items=[];
